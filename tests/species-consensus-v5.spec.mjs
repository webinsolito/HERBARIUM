...
test.describe('species consensus v5 arbitration',()=>{
 ...
 test('requires both tuned score and separation margin',()=>{expect(decideSpeciesConsensusV5(p('A a',.99,.79),p('B b',.9,.4)).status).toBe('UNKNOWN');expect(decideSpeciesConsensusV5(p('A a',.919,.95),p('B b',.9,.4)).status).toBe('UNKNOWN');});
 test('recovers benchmark sunflower-like high-separation disagreement',()=>{const x=decideSpeciesConsensusV5(p('Helianthus annuus',.927,.877),p('Calendula officinalis',.8,.2));expect(x.status).toBe('PROPOSED');expect(x.arbitrated).toBe(true);});
 ...
 test('policy keeps a hard floor above the known hot-dog negative',()=>{expect(ARBITRATION_POLICY.strongPrimaryTop1).toBeGreaterThan(.881);expect(ARBITRATION_POLICY.strongPrimaryMargin).toBeGreaterThanOrEqual(.80);});
});
