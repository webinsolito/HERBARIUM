...
assert.match(app,/indexedDB\.open\(DB_NAME,1\)/);assert.match(app,/objectStore\(STORE_NAME\)\.add\(observation\)/);assert.match(app,/objectStore\(STORE_NAME\)\.delete\(id\)/);assert.match(app,/Verifica del salvataggio locale fallita/);assert.match(app,/Verifica eliminazione fallita/);assert.match(app,/storage\.estimate/);assert.match(app,/storage\?\.persist/);assert.match(app,/MAX_PIXELS=40_000_000/);assert.match(app,/inspectImageFile/);assert.match(app,/exifStripped:true/);assert.match(app,/const data=await blob\.arrayBuffer\(\)/);assert.match(app,/new Blob\(\[payload\]/);assert.match(app,/identification:\{status:'UNAVAILABLE',species:null,confidence:null\}/);assert.match(app,/assessEvidenceQuality/);assert.match(app,/createSpeciesRecognitionAdapter/);assert.match(app,/speciesEngine:\{/);assert.match(app,/createAutomaticNonPlantAdapter/);assert.match(app,/classifyPixelsLocally\(\{evidence\},automaticGateAdapter\)/);assert.match(app,/location\.assign\(.*result\.html/);assert.match(app,/item\.status==='REJECT'/);assert.match(app,/item\.status==='VERIFIED'/);assert.doesNotMatch(app,/status\s*:\s*['"]VERIFIED['"]/);assert.doesNotMatch(app,/innerHTML\s*=/);assert.doesNotMatch(app,/https?:\/\//);
