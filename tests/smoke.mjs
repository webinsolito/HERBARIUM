...
const bellis=read('src/species-bellis.html');
const immersive=read('src/immersive.css');
const plant3d=read('src/plant3d.js');
...
assert.match(home,/id="plantCanvas"/);assert.match(home,/immersive\.css/);assert.match(home,/plant3d\.js/);assert.match(home,/Scopri cosa hai davanti/);assert.match(home,/Scatta foto/);assert.match(home,/Carica foto/);assert.match(home,/species-bellis\.html/);assert.match(home,/Natural Atlas 3D/);
assert.doesNotMatch(home,/id="whole"|id="collectionList"|id="pendingList"|id="atlasRegions"|id="academyList"/);
assert.match(observe,/accept="image\/\*"/);assert.match(observe,/capture="environment"/);assert.match(observe,/id="whole"/);assert.match(observe,/id="flower"/);assert.match(observe,/id="leaf"/);assert.match(observe,/id="detail"/);assert.match(observe,/Aggiungi una foto per iniziare/);
assert.match(observe,/id="negativeSignal"/);for(const category of ['object','animal','print','tablecloth'])assert.match(observe,new RegExp(`value="${category}"`));assert.match(observe,/Scatta una foto/);assert.match(observe,/Scegli dalla libreria/);assert.match(observe,/Aggiungi altri dettagli/);assert.match(observe,/La foto non mostra una pianta/);assert.match(observe,/Le foto restano sul dispositivo/);
assert.match(collection,/id="collectionList"/);assert.match(collection,/Le tue osservazioni/);assert.match(collection,/botanical-sprig\.svg/);assert.match(collection,/class="mobile-tabbar"/);assert.match(book,/id="pendingList"/);assert.match(book,/Flora Observata/);assert.match(book,/Pagine validate/);assert.match(book,/botanical-sprig\.svg/);assert.match(book,/class="mobile-tabbar"/);assert.match(atlas,/id="atlasRegions"/);assert.match(atlas,/Nessun cloud/);assert.match(atlas,/Privacy geografica/);assert.match(academy,/id="academyList"/);assert.match(academy,/id="academyObservationCount"/);assert.match(academy,/id="academyMultiViewCount"/);assert.match(academy,/Nessun corso fittizio/);
assert.match(css,/@media\(max-width:520px\)[\s\S]*\.capture\{grid-template-columns:1fr\}/);assert.match(css,/\.capture input\{[^}]*width:100%/);
assert.match(immersive,/--bg:#08110d/);assert.match(immersive,/\.plant-canvas\{/);assert.match(immersive,/\.species-left/);assert.match(immersive,/\.viewer-controls/);assert.match(immersive,/@media\(max-width:900px\)/);
assert.match(js,/indexedDB\.open\(DB_NAME,1\)/);assert.match(js,/objectStore\(STORE_NAME\)\.add\(observation\)/);assert.match(js,/objectStore\(STORE_NAME\)\.getAll\(\)/);assert.match(js,/item=>item&&item\.status==='UNKNOWN'/);assert.match(js,/Foto pronta/);assert.match(js,/Da verificare/);assert.match(js,/observationImageUrl/);assert.match(js,/book-specimen-card/);assert.match(js,/status:negativeGate\.status/);assert.match(js,/negativeCategory:negativeGate\.negativeCategory/);assert.match(js,/currentNegativeGate=\(\)=>classifyNegativeEvidence\(negativeSignal\?negativeSignal\.value:null\)/);assert.match(js,/negativeSignal\.addEventListener\('change',update\)/);assert.doesNotMatch(js,/classifyNegativeEvidence\(null\)/);
...
assert.match(bellis,/Bellis perennis/);assert.match(bellis,/Demo catalogo 3D/);assert.match(bellis,/data-action="explode"/);assert.match(bellis,/id="partRail"/);assert.match(bellis,/id="hotspots"/);assert.match(bellis,/Non collegato/);
assert.match(plant3d,/getContext\('webgl'/);assert.match(plant3d,/toggleExplode/);assert.match(plant3d,/pointerdown/);assert.match(plant3d,/wheel/);for(const part of ['petal','head','involucre','stem','leaves','roots','seeds'])assert.match(plant3d,new RegExp(part));assert.doesNotMatch(plant3d,/fetch\(|XMLHttpRequest|https?:\/\//);
