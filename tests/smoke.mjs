...
assert.match(js,/renderAcademy/);assert.match(js,/roles\.length>=2/);assert.match(js,/Nessuna osservazione reale disponibile/);assert.match(js,/Impossibile leggere i dati locali dell’Academy/);assert.match(js,/screen==='academy'/);assert.match(js,/await renderAcademy\(\)/);
assert.match(js,/await renderCollection\(\)/);assert.match(js,/SCREEN_IDS/);assert.match(js,/querySelectorAll\('\.screen'\)/);assert.match(js,/history\.pushState/);assert.match(js,/window\.scrollTo\(0,0\)/);assert.match(js,/showScreen\(location\.hash\.slice\(1\)\|\|'home'/);assert.match(js,/screen==='collection'/);assert.match(js,/renderStats\(\);renderCollection\(\);renderBook\(\);renderAtlas\(\);renderAcademy\(\)/);
