...
const sunflower=path.resolve('tests/species-assets/sunflower.png');
...
async function analyse(page,file){
  ...
  return page.evaluate(()=>new Promise((resolve,reject)=>{
    ...
    request.onsuccess=()=>{
      const db=request.result,q=db.transaction('observations','readonly').objectStore('observations').getAll();
      q.onsuccess=()=>{resolve((q.result||[]).at(-1)||null);db.close();};
...
test.describe('P0-B real species runtime',()=>{
  test.setTimeout(300000);
  test('PlantNet ONNX executes locally without forcing VERIFIED',async({page})=>{
    await clearDb(page);
    const item=await analyse(page,sunflower);
    expect(item).toBeTruthy();
    expect(item.status).not.toBe('VERIFIED');
    expect(item.analysis?.automaticGate?.status).not.toBe('REJECT');
    expect(item.analysis?.speciesEngine?.version).toBe('plantnet300k-mobilenetv3-onnx-v1');
