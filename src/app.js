(() => {
  ...
  const prepareEvidenceImage = async file => {
    ...
    if (file.size && file.size > MAX_INPUT_BYTES) throw new Error('La foto selezionata supera il limite locale consentito.');
  ...
  const validateSelection = () => {
    for (const input of selectedInputs()) {
      ...
      if (file.type && !file.type.startsWith('image/')) return "Il file selezionato non è un'immagine.";
      if (file.size && file.size > MAX_INPUT_BYTES) return 'La foto selezionata supera 12 MB: scegli una foto più leggera.';
    }
    return null;
  };
  ...
  inputs.forEach(input => input.addEventListener('change', () => {
    ...
    if (error) {
      ...
      setResult(`UNKNOWN — ${error} Nessun dato è stato salvato.`);
      return;
    }
    update();
  }));
  ...
  analyse.addEventListener('click', async () => {
    ...
    if (selectionError) {
      analyse.disabled = true;
      setResult(`UNKNOWN — ${selectionError} Nessun dato è stato salvato.`);
      return;
    }
    analyse.disabled = true;
    setResult('Preparazione e salvataggio locale delle prove fotografiche…');
    try {
      ...
      setResult('UNKNOWN — prove fotografiche ottimizzate e salvate nel database locale del dispositivo; nessuna identificazione automatica simulata.');
      await renderStats();
    } catch (error) {
      console.error(error);
      setResult('UNKNOWN — salvataggio locale non riuscito. Nessuna osservazione incompleta è stata registrata. Controlla lo spazio disponibile e riprova.');
    } finally {
      const selectionErrorAfterSave = validateSelection();
      if (selectionErrorAfterSave) analyse.disabled = true;
      else if (selectedInputs().length) analyse.disabled = false;
    }
  });
  ...
  const setNetwork = () => {
    ...
    network.textContent = online ? 'online · dati locali' : 'offline · dati locali';
    network.setAttribute('aria-label', online
      ? 'Connessione disponibile. Le osservazioni restano memorizzate localmente.'
      : 'Senza connessione. Le funzioni locali restano disponibili.');
  };
